...
public class ResourceClassifier {
    ...
    private String targetPathFor(String originalPath, boolean war) {
        ...
        if (shouldSkipSpringBootLoaderService(originalPath)) {
            return SKIPPED;
        }
    ...
    private String noteFor(String originalPath, ResourceFinding.Category category, String targetPath) {
        if (SKIPPED.equals(targetPath)) {
            ...
            if (shouldSkipSpringBootLoaderService(originalPath)) {
                return "Spring Boot loader filesystem provider service is generated by the boot repackage step.";
            }
    ...
    private boolean shouldSkipSpringBootLoaderService(String path) {
        return SPRING_BOOT_FILESYSTEM_PROVIDER_SERVICE.equals(path);
    }
