...
- `artifact_*` columns compare the original artifact with the Maven package produced from the restored source project. jar2mp preserves original `META-INF/maven/**` metadata, package manifests where Maven can safely consume them, and retained build-info/SBOM resources while disabling generated Maven descriptors or metadata generators that would overwrite those entries. This track isolates remaining compiler, manifest, resource, nested-library, or packaging drift.
