...
Runtime and artifact evidence are reported as separate gate columns. Runtime event counts are de-duplicated by `kind/owner/target/value`. Long-running web applications that collect trace events and respond to the local HTTP startup probe before the timeout are marked with runtime gate `PASS_HEALTHY_TIMEOUT`; timeouts that collect events without health evidence remain `WARN_STARTED_TIMEOUT`, while Spring Boot startup failure output is reported as run status `STARTUP_FAILED_TIMEOUT` and runtime gate `FAIL_STARTUP_FAILED_TIMEOUT`. Rebuilt source-package byte differences are also warning-level evidence, not proof of byte-identical source recompilation. Library JARs, thin JARs without a runnable manifest, and standard WARs can report `UNSUPPORTED` launch support with runtime gate `SKIPPED_UNSUPPORTED`.
