...
Runtime and artifact evidence are reported as separate gate columns. Runtime event counts are de-duplicated by `kind/owner/target/value`; automatic smoke runs scope the trace agent to the entrypoint package plus `org.springframework.boot.loader.` to reduce large Spring Boot startup transformation cost. Long-running web applications that collect trace events and respond to the local HTTP startup probe before the timeout are marked with runtime gate `PASS_HEALTHY_TIMEOUT`; timeouts that collect events without health evidence remain `WARN_STARTED_TIMEOUT`, while Spring Boot startup failure output is reported as run status `STARTUP_FAILED_TIMEOUT` or `STARTUP_FAILED_EXIT` and runtime gate `FAIL_STARTUP_FAILED_*`. Redis/connection-refused external dependency startup failures are recorded as zero-impact `runtime_environment` observations and do not lower the byte-level restoration score or remaining restoration gap count; other startup failure statuses are treated as the terminal runtime gap, so static file/socket/resource calls that the failed launch did not reach are not listed as separate missing-kind gaps. Rebuilt source-package byte differences are also warning-level evidence, not proof of byte-identical source recompilation. Library JARs, thin JARs without a runnable manifest, and standard WARs can report `UNSUPPORTED` launch support with runtime gate `SKIPPED_UNSUPPORTED`.

Decompiler parity risk counts are advisory source-review signals. When build, source coverage, byte package, and source-recompiled class-byte gates pass, those risk counts are not remaining restoration gaps.

For source comparison against a reference checkout, shared Java text diffs are split into format-only, import-only, decompiler-artifact, byte-equivalent-text, and substantive buckets. `byte-equivalent-text` means the source text still differs from the reference checkout while the generated source recompiles to the original class bytes. The resolved content-diff count sums every non-substantive bucket so the remaining source repair surface is visible separately from harmless or byte-equivalent text drift.

The script now records four artifact-fidelity tracks:

- `artifact_*` columns compare the original artifact with the Maven package produced from the restored source project. jar2mp overlays original class bytes during `process-classes`, preserves original entry mtimes for copied class/resource bytes, keeps original `META-INF/maven/**` metadata, package manifests where Maven can safely consume them, and retained build-info/SBOM resources while disabling generated Maven descriptors or metadata generators that would overwrite those entries. This track reports `artifact_content_entries_match` separately from `artifact_archive_bytes_same`; `source_artifact_gate=PASS_CONTENT` means every non-directory entry's bytes match and only the ZIP container bytes still differ. Per-project `artifact-fidelity-summary.csv` files also append ZIP-container diagnostics such as `archive_entry_order_same`, `archive_metadata_diff_entries`, timestamp differences, compression-method differences, compressed-size differences, and extra-field differences so container-only drift can be triaged separately from content drift. When content matches but entry order, empty directory entries, or in-place ZIP metadata differs, `--compare-artifact` also writes an `archive-order-restored/` candidate by reordering rebuilt ZIP records, normalizing empty directory-only entries, and restoring same-length ZIP timestamps/extra fields without recompressing them; the real-world summary promotes that result as `artifact_archive_order_restored_exact` alongside the other `artifact_archive_*` columns.
...
- `byte_exact_*` columns run a separate `--byte-exact-package --verify-build` restoration and compare the generated project's final Maven `package` output against the original artifact. This is the strict package-level byte equality gate.
- `package_record_restore_*` columns run a separate guarded `--restore-package-records --verify-build` restoration. This keeps the ordinary generated POM shape, then reports whether final package record replay became exact after the helper proved rebuilt non-directory entries and payload digests match the original.

For strict byte-level package restoration, run jar2mp with `--byte-exact-package`. That mode implies `--emit-raw-artifact`, uses the original artifact base name as Maven `finalName`, adds common test and quality plugin skip properties, and skips package-transforming shade/assembly/repackage plugins from the original POM so the standalone helper can own the final artifact restoration step. jar2mp also writes that helper under `.jar2mp/byte-exact/` and a `.jar2mp/byte-exact/raw-artifact/<original-name>` reference artifact; the generated Maven project runs the helper during `package` after the rebuilt package succeeds, then restores the final output from the original ZIP records, including entry order, empty directories, manifest/module-info/resource/class payloads, entry-set drift, and ZIP metadata. When combined with `--verify-build`, its default verification goal is `package` unless `--verify-goal` is set explicitly, and jar2mp writes `target/byte-exact-package-check/` fidelity reports for the final package output. Use `--restore-package-records` when you want the ordinary source-rebuild project to keep its normal `finalName` and plugin set, but to overwrite the final package only after the helper confirms the rebuilt non-directory entry set and payload digests match the original; that guarded path writes `target/package-record-restore-check/`. The realworld script runs byte-exact mode separately from the normal source-rebuild package check so `byte_exact_*` proves strict package-level restoration while `artifact_*` continues to measure how close recompilation from decompiled sources is.
...
- per-project byte-exact reference artifacts under `target/realworld-samples/restored/<sample>-byte-exact/<artifactId>/.jar2mp/byte-exact/raw-artifact/`
