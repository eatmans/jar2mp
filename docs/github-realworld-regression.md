...
For strict byte-level package restoration, run jar2mp with `--byte-exact-package`. That mode implies `--emit-raw-artifact`, uses the original artifact base name as Maven `finalName`, adds common test and quality plugin skip properties, and skips package-transforming shade/assembly/repackage plugins from the original POM so the standalone helper can own the final artifact repair step. jar2mp also writes that helper under `.jar2mp/byte-exact/` and a `.jar2mp/byte-exact/raw-artifact/<original-name>` reference artifact; the generated Maven project runs the helper during `package` to repair the rebuilt output's ZIP records, including manifest/module-info payloads and original-only or rebuilt-only entry-set drift, without directly copying the raw artifact over the final package. When combined with `--verify-build`, its default verification goal is `package` unless `--verify-goal` is set explicitly, and jar2mp writes `target/byte-exact-package-check/` fidelity reports for the final package output. The realworld script runs this mode separately from the normal source-rebuild package check so `byte_exact_*` proves strict package-level restoration while `artifact_*` continues to measure how close recompilation from decompiled sources is.
