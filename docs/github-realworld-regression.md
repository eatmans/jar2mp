...
- `artifact_*` columns compare the original artifact with the Maven package produced from the restored source project. jar2mp overlays original class bytes during `process-classes`, preserves original `META-INF/maven/**` metadata, package manifests where Maven can safely consume them, and retained build-info/SBOM resources while disabling generated Maven descriptors or metadata generators that would overwrite those entries. This track isolates remaining resource, nested-library, metadata, or ZIP container drift.
