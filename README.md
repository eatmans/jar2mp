...
- `gap-summary.md` - 主要缺口汇总；存在 source rebuild fidelity CSV 时同样会显示源码重编译 class 字节一致性摘要；存在 package fidelity CSV 时同样会显示最终 package exact/archive/content 摘要；运行时启动失败缺口会包含 failure message 和首个 `Caused by:` 原因
...
4. 看 `restoration-score.md` 和 `gap-summary.md` 了解源码、资源、运行时观测与构建验证缺口；如启用了 `--verify-build` 且构建成功，两份报告都会展示 source rebuild class 字节一致性摘要
